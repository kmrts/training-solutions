...
public class Bottle {  //senior
    // amelynek van 2 attribútuma: BottleType type és int filledUntil.
    // A BottleType legyen felsorolásos típus az alábbi értékekkel: GLASS_BOTTLE és PET_BOTTLE, .
    // A Bottle nevű osztálynak legyen egy static metódusa of néven, mely paraméterként egy BottleType-ot
    // vár és visszaad egy új Bottle objektumot amelyet a megadott paraméter alapján hoz létre.
...
//        this.filledUntil = filledUntil;
    ...
    public static Bottle of(BottleType bt){
        return new Bottle(bt);
    }
    ...
    public void fill(int fillAmount){
        ...
        if(filledUntil+fillAmount> type.getMaximumAmount()){
            throw new IllegalArgumentException("over maximum");
        }
